...
代理全部在**控制台「总览 → 代理设置」**里管理（保存后立即生效，无需重启，持久化到 `/data/proxies.json`）：

- 文本框里**一行一个代理**（`http://...` / `socks5://...`），保存即生效；
- 每个账号自动按稳定哈希分配到池内一个代理（同一账号同一出口，session IP 稳定），连接失败自动回落池内下一个；
- 「测试」可验证任意代理或已配置代理：显示**出口 IP / 国家 / 延迟 / codebuff 状态**，一眼确认是否生效；
- 账号列表每行显示实际生效的**出口**；清空保存 = 清除全局池，走环境变量/直连。

> 地址怎么填：默认 bridge 网络，代理在**另一台机器**填真实 IP（`http://192.168.1.10:2334`）；
> 代理在**本机**且监听 0.0.0.0（Clash 开 Allow LAN）时填 docker 网关 IP（`docker network inspect bridge` 查，通常 172.17.0.1）。
> 容器内 `127.0.0.1` 是容器自己，不代表宿主机代理。
