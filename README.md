...
- **新会话**从健康账号里选「当前持有会话数最少」的账号，平局按轮询次序轮流
  ...
  导致新会话总落到同一个账号；
- **同一会话**在**记忆 TTL 内**（默认 60s，`lb.sticky_ttl_sec` 可调）固定走该账号（热 session 复用），
  **过期后重新轮询分配**——避免恒定 `conversation_id` 把账号钉死、永远不轮巡
  （`sticky_ttl_sec: 0` = 完全不做记忆，每请求重新轮询）；账号冷却/超额/报错时自动回落池子；
