...
- 会话 ID 自动从请求里提取，优先级：`codebuff_metadata.client_id`（Codebuff/Freebuff
  原生会话 ID）→ 显式会话字段（`conversation_id` / `thread_id` / `session_id`）→ 常见会话头
  （`x-conversation-id` / `x-thread-id` / `x-session-id`）→ `user` 字段兜底。
- **新会话**用 FNV-1a 稳定哈希从健康账号里选种子账号（冷却中 / 已用满限额的账号不参与），
  4 个账号时 4 个新对话会自然摊到 4 个账号，而不是全部压在一个账号上；
- **同一会话**后续请求固定走该账号（热 session 复用，避免 `superseded` 与重复 admit）；
  该账号冷却/超额时自动回落到池子，并记住新账号；
- 无会话 ID 的请求（如控制台 playground）走池子轮询 + 配额感知选号。
...
- **自动跟随（auto，默认）**：会话级负载均衡——同一会话固定同一账号，不同会话按哈希摊到不同账号；账号冷却时自动回落到池子并记住新账号。
