...
- **会话临近过期提前切换（按模型分层）**：剩余时间低于提前量阈值的会话不再承接新请求，
  ...
  - **免费模型**：`session.free_model_re_admit_lead_sec`（默认 60s = **1 分钟**）——
    会话剩余不足 1 分钟即**不再调度到该会话**，提前 re-admit 换新会话（只留**最小**
    切换余量，避免请求打到即将过期的会话上被掐断；切换本身会退还未用时长）；
...
- 控制台「总览」每个账号有两列额度：
  ...
  **Freebucks** = 余额 `N FB` / 当前模型单价 `N/h` / **折算可用时长**（如 ≈`30 分钟`）
  / 今日池 `剩余/上限`，悬停可看钱包余额、计费方式与最近一次早退回执（实退 / 应付）。
...
- `rateLimitsByModel.recentCount` 可能是小数——这是 **session_units** 那本账：admit 时先
  预占 1.0，提前释放后按**实际占用时长**重算（如 1.2 → 0.3）。
- **Freebucks 同样按实际占用退还**：提前 DELETE 的回执带 `freebucksRefund`；
  `freebucksRefundPending` 表示**结算未完成**（要用同一个 `instanceId` 重放 DELETE 取回执），
  **不是"不退"**。所以空闲早退既**释放会话槽位**，也**停止为空转时长付费**。
...
计费按**整小时单价预扣**：`admit` 时扣一个整小时单价，提前 `DELETE` 会把**未用部分按实际
占用时长退回来**（2026-09-13 结论反转，见 §3）。所以挂着的空闲会话是在花钱，代理做了五件事：

- **空闲自动释放（默认 60s = 1 分钟，可调 5s..24h）**：会话在途请求归零后开始计时，空闲超过该时长
  立即早退 `DELETE`——必须带 `x-freebuff-instance-id`，否则上游 400 `instance_required`，
  会话既删不掉，那笔预扣的钱也追不回来。
  **它同时做两件事**：释放上游会话槽位（一个账号同时只有一条 session 且绑定模型），
  以及**停止为空转时长付费**（未用部分会退还）。所以默认值从 600s 改回 60s。
  上游回 `freebucksRefundPending`（结算未完成）时，本服务会用同一个 instanceId
  **持续重放 DELETE 追问**（进程内 30s 一次 + 常驻周期扫尾），直到拿到终态回执。
  控制台会按账号池实时算推荐值并可一键采用（见下文）。
  `0` = 关闭释放（会话留到自然过期；代价是空转时长照付 + 换模型要等）。后台轮询不会顺延这个计时。
...
- **换号即释放**：账号级故障换号前，先把失败账号的会话早退 DELETE（结束会话、释放上游
  ...
  >
  > 因此：**早退既省槽位也省钱**，本服务会用同一个 `instanceId` **持续重放 DELETE**
  > 直到拿到终态回执（含 0——0 才是真的退 0）。
  > 完整证据链与纠错过程见
