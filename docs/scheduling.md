...
- **会话临近过期提前切换（按模型分层）**：剩余时间低于提前量阈值的会话不再承接新请求，
  ...
  - **免费模型**：`session.free_model_re_admit_lead_sec`（默认 60s = **1 分钟**）——
    ...
    切换余量，避免请求打到即将过期的会话上被掐断；一次 admit 买断一小时）；
...
计费按**整小时单价预扣**：一次 `admit` = **买断一小时**（当场扣满，回执带 `expiresAt`）。
这一小时内继续发请求**边际成本为 0**；而提前 `DELETE` 之后那一小时就作废，重开 = 重新买一整小时。
所以「释放能把 Freebucks 省回来」**并不成立**，代理据此做了这些事：

- **空闲自动释放（默认 60s = 1 分钟，可调 5s..24h）**：`DELETE` 必须带
  `x-freebuff-instance-id`，否则上游 400 `instance_required`，会话删不掉。
  **付费时段内不释放**——闲置不花钱，释放才是把已买的钱丢掉。本项现在是**付费时段结束之后**
  的空闲释放时长，到期即释放，**腾出上游会话槽位**（一个账号同时只有一条 session 且绑定模型）。
  上游回 `freebucksRefundPending`（结算未完成）时，本服务会用同一个 instanceId
  **持续重放 DELETE 追问**（进程内 30s 一次 + 常驻周期扫尾），直到拿到终态回执——
  但**不要**把 pending 当成"钱会回来"来决策释放时机。
  控制台会按账号池实时算推荐值并可一键采用（见下文）。
  `0` = 关闭释放（会话留到自然过期；代价是换模型要等）。后台轮询不会顺延这个计时。
