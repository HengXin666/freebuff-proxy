...
- **换号不再"每个账号都买一条计费会话"**（2026-09 Freebucks 改版）：上游是**整小时买断**
  ——admit 一次就按模型单价（N/h）预扣整小时，**早退 DELETE 不退 Freebucks**
  （2026-09-13 实测结案，见 [account-scheduling-and-refund.md](./account-scheduling-and-refund.md) §3）。
  ...
  **请求彻底失败时也会早退释放**，不让会话白占槽位。详见下方「额度保护（Freebucks 计费，控制台可调）」。
...
- **会话临近过期提前切换（按模型分层）**：剩余时间低于提前量阈值的会话不再承接新请求，
  ...
  - **免费模型**：`session.free_model_re_admit_lead_sec`（默认 60s = **1 分钟**）——
    会话剩余不足 1 分钟即**不再调度到该会话**，提前 re-admit 换新会话（2026-09 起是整小时
    买断：re-admit = 再买一小时，所以只留**最小**切换余量，避免请求打到即将过期的会话上被掐断）；
...
- 控制台「总览」每个账号有两列额度：
  ...
  **Freebucks** = 余额 `N FB` / 当前模型单价 `N/h` / **折算可用时长**（如 ≈`30 分钟`）
  / 今日池 `剩余/上限`，悬停可看钱包余额、计费方式与最近一次早退回执（退款实测恒为 0）。
...
- `rateLimitsByModel.recentCount` 可能是小数——这是 **session_units** 那本账：admit 时先
  预占 1.0，提前释放后按**实际占用时长**重算（这本账**会**退还，如 1.2 → 0.3）。
  但它与 Freebucks 是**两套独立的账**：Freebucks 早退不退。因此省额度只能靠
  **复用热 session（少 admit）**；空闲早退是为了**释放会话槽位**，不是为了省钱。
...
计费是**整小时买断**：`admit` 一次就扣一个整小时单价，之后这条会话用 3 秒还是 59 分钟
扣的都一样多（早退不退 Freebucks，见 §3 结案）。所以**花钱次数 = admit 次数**，
省钱的唯一办法是**少开会话、尽量复用同一条热会话**。代理做了五件事：

- **空闲自动释放（默认 600s = 10 分钟，可调 5s..24h）**：会话在途请求归零后开始计时，空闲超过该时长
  立即早退 `DELETE`——必须带 `x-freebuff-instance-id`，否则上游 400 `instance_required`，
  会话既删不掉也拿不到 session_units 退还。
  **注意它现在的作用是「释放上游会话槽位」，不是「省钱」**：早退不退钱，释放后再来请求就是
  **又买一小时**。所以默认值从 60s 上调到 600s，把交互式对话的停顿留在同一会话里。
  控制台会按账号池实时算推荐值并可一键采用（见下文）。
  `0` = 关闭释放（会话留到自然过期，最省 admit；代价是换模型要等释放）。后台轮询不会顺延这个计时。
...
- **换号即释放**：账号级故障换号前，先把失败账号的会话早退 DELETE（结束会话、释放上游
  会话槽位，让别的账号/模型能拿到 slot）。
  > ✅ **退款悬案已结案（2026-09-13）**：早退 DELETE **只退还 `session_units`**
  > （每日模型额度，按实际占用时长重算），**Freebucks 一分不退**。
  >
  > 受控实验（账号 `loliyoknvrgq`、模型 `upstage/solar-pro4`、占用 180s）：
  > 预扣 `session_units` **0.2 → 1.2** 且 Freebucks **+5**；DELETE 后每 3 秒重放
  > **100 次、共 555 秒**，`freebucksRefundPending` 恒为 true、**金额字段始终没出现**；
  > 终态 `session_units` 退到 **0.3**（退了 0.9），Freebucks **25/25 未退**。
  > 换模型重放 200 次 / 1195 秒，结果一致。
  >
  > ⚠️ 本文旧版曾把 0 解释成"我们问得太早、且没再去问"（依据是官方源码注释里的
  > *"REFUNDS the unused window"*）——**该解释已证伪**：那句话讲的是 `session_units`
  > 这本账，不是 Freebucks。上游有**两套独立的账**，详见 §3.1/§3.3。
  >
  > 因此：**省钱只能靠"少开会话"（少 admit），不能靠"早退"**。早退仍保留
  > （确认会话结束、释放槽位、不留 orphan），但**绝不为了多拿退款去增加 admit/DELETE 次数**。
  > 完整证据链、两套账的对照表与决定性实验协议见
